...
export default function EditorMovimentacao({ item, tipo, categorias = [], onFechar, onSalvar }) {
    ...
    return <div className="mov-modal-overlay" onMouseDown={(event) => event.target === event.currentTarget && onFechar()}><form className="mov-modal" onSubmit={salvar} role="dialog" aria-modal="true">
        ...
        <div className="mov-modal-grid">
            ...
            <label><span>Categoria *</span><select name="id_categoria" defaultValue={item.id_categoria ?? ""} required><option value="" disabled>Selecione</option>{categorias.map((opcao) => <option key={opcao.id} value={opcao.id}>{opcao.nome}</option>)}</select></label>
