...
export default function Despesas({ usuario, apiUrl, despesas = [], onAtualizar, onExcluir, onLogout }) {
    ...
    const filtradas = useMemo(() => {
        ...
        return termo ? despesas.filter((item) => [item.descricao, item.fornecedor, item.projetoNome, item.categoria].some((valor) => String(valor ?? "").toLocaleLowerCase("pt-BR").includes(termo))) : despesas;
    }, [busca, despesas]);
    ...
    return <div className="app"><Sidebar paginaAtiva="Despesas" tipoUsuario={usuario.tipo} onLogout={onLogout} /><div className="main"><Header usuario={usuario} /><main className="entradas-content">
        ...
        <section className="entradas-table-card">{filtradas.length === 0 ? <div className="entradas-empty"><ReceiptText size={26} /><strong>{busca ? "Nenhuma despesa encontrada" : "Nenhuma despesa registrada"}</strong><span>{busca ? "Tente buscar usando outro termo." : "As despesas aparecerão aqui após o primeiro registro."}</span></div> : <div className="entradas-table-scroll"><table><thead><tr><th>DATA</th><th>DESCRIÇÃO</th><th>FORNECEDOR</th><th>PROJETO</th><th>VENCIMENTO</th><th>VALOR</th><th>FORMA</th><th>STATUS</th></tr></thead><tbody>{filtradas.map((item) => <tr key={item.id} className="mov-row-clickable" tabIndex="0" role="button" onClick={() => setSelecionada(item)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setSelecionada(item); } }}><td>{item.data ? data.format(new Date(`${item.data}T12:00:00`)) : "-"}</td><td><strong>{item.descricao || "-"}</strong></td><td>{item.fornecedor || "-"}</td><td>{item.projetoNome || "-"}</td><td>{item.vencimento ? data.format(new Date(`${item.vencimento}T12:00:00`)) : "-"}</td><td className="despesa-value">- {moeda.format(Number(item.valor || 0))}</td><td>{formasPagamento[item.formaRecebimento] ?? "-"}</td><td><span className={`despesa-status status-${item.status}`}>{statusDespesa[item.status] ?? "-"}</span></td></tr>)}</tbody></table></div>}</section>
